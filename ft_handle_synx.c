...
/*   Updated: 2024/09/20 12:16:46 by rel-mora         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if((start)->type == '<' ||(start)->type == '>' ||(start)->type == DREDIR_OUT || (start)->type == HERE_DOC )
		return 1;
	return 0;
}
...
int	ft_check_between(t_splitor **start)
{
	while ((*start) != NULL)
	{
		if (redirection((*start)) && ((*start)->state == G))
		{
			...
			if ((*start) == NULL || ((redirection(*start) || (*start)->type == '|') && (*start)->state == G))
			{
				return (1);
			}
		}
